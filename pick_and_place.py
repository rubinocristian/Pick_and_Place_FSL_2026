...
class DOFBOTController:
    ...
    def __init__(self):
        # LUNGHEZZE BRACCI REALI (da tue misurazioni)
        self.l1 = 0
        self.l2 = 9.0
        self.l3 = 9.0
        self.l4 = 19.0
    ...
    def inverse_kinematics(self, x: float, y: float, z: float, cm: float):
        ...
        def cinematica_inversa_3link(distanza, altezza):
            ...
            if r > portata_max:
                raise ValueError(
                    f"Distanza {r:.2f} cm oltre la portata massima di {portata_max} cm"
                )
            ...
            def cinematica_directa(thetas_deg):
                ...
                alpha1 = np.radians(90) - delta1
                alpha2 = alpha1 - delta2
                alpha3 = alpha2 - delta3
            ...
            return (
                round(np.clip(best_theta[0], 0, 180), 1),
                round(np.clip(best_theta[1], 0, 180), 1),
                round(np.clip(best_theta[2], 0, 180), 1),
            )

        theta_2, tetha_3, tetha_4 = cinematica_inversa_3link(
            distanza=d, altezza=z
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        angles = robot.move_to_position(x=15, y=15, z=10, cm=4.3)
        ...
        arm.Arm_serial_servo_write6(
            angles[0], angles[1], angles[2], angles[3], angles[4], angles[5], 2000
        )
